...
description: Gemini Web access workflow for text generation, image generation, vision input, and multi-turn sessions via a reverse-engineered API, with explicit user consent required. Use when Gemini-Web-specific behavior or a backend dependency is explicitly needed. Prefer `baoyu-image-gen` or more specific generation skills for normal creation.
---
