...
### → personal-dossier-builder（已集成）
本 skill 的 4 种提问方法（关键事件/对比逼近/Laddering/反事实）已注入 `personal-dossier-builder` 的 interview 模式。
当补档访谈遇到「说不清」「差不多」时，自动切换到行为挖掘模式。
本 skill 保持独立运行，dossier-builder 调用的是方法论子集。

### → continuous-learning-v2（已集成）
本 skill 的推断方法（对比/边界/Laddering）为 `continuous-learning-v2` 的 Pattern Detection 提供「为什么」推断层。
当 observer 发现同一行为模式 ≥3 次时，自动触发 tacit 推断，生成带 `inferred_rule` 的增强 instinct。
本 skill 保持独立运行，continuous-learning-v2 调用的是推断逻辑子集。
