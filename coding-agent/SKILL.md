...
## 🛡️ Coding Discipline (From Anthropic's Claude Code System Prompt)

When orchestrating or prompting these coding agents, enforce these strict disciplines:

1. **极致克制 (No Gold-Plating)**:
   - "Don't add features, refactor code, or make 'improvements' beyond what was asked."
   - "Don't add error handling or fallbacks for scenarios that can't happen. Trust internal code guarantees."
2. **不碰未读代码 (Read First)**:
   - "Do not propose changes to code you haven't read. If a user asks about or wants you to modify a file, read it first."
3. **必须自测闭环 (Verify Before Reporting)**:
...
4. **最大化并发 (Maximize Parallelism)**:
   - "If you intend to call multiple independent tools, make all tool calls in parallel to increase efficiency."
5. **禁用野生 Bash (Use Dedicated Tools)**:
   - "Do NOT use the bash tool to run commands when a relevant dedicated tool (like file read/write, glob, grep) is provided."
